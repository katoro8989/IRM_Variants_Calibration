...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        print(f'optimizer :{self.hparams["optimizer_name"]}')
        ...
        self.optimizer = build_optimizer(
            OptimizerSetting(
                ...
                model=self.network,
                momentum=self.hparams["momentum"], # sgd, momentum_sgd, sgd_nesterov, rmsprop
                eps=self.hparams["eps"], # adam, rmsprop (term added to the denominator to improve numerical stability)
                alpha=self.hparams["alpha"], # rmsprop(smoothing constant of 2nd moment)
                beta_1=self.hparams["beta_1"], # adam (smoothing constant of 1st moment)
                beta_2=self.hparams["beta_2"],  # adam (smoothing constant of 2nd moment)
                rho=self.hparams["rho"], # SAM
            )
        )
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        if self.hparams["optimizer_name"] == 'sam':
            self.optimizer.zero_grad()
            loss.backward()
            self.optimizer.first_step(zero_grad=True)
            second_loss = F.cross_entropy(self.predict(all_x), all_y)
...
class fIRMGAME(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(fIRMGAME, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        
        self.num_domains = num_domains
        self.num_classes = num_classes
        self.period = self.num_domains
        ...
        for e in range(self.num_domains):
            ...
            network = nn.Sequential(featurizer, classifier).to('cuda:0')
            network = nn.DataParallel(network)
            self.model_list.append(network)
    ...
    def update(self, minibatches, unlabeled=False):
        loss_ave = 0.  # average of loss values
            
        e = self.update_count % self.period
        # for e in range(self.num_domains):
    ...
    def predict(self, x):
        y_ = torch.zeros(x.size()[0], self.num_classes, device=x.device) #batchsize * num_classes
        for e in range(self.num_domains):
            y_ = y_ + self.model_list[e](x)
            
        return y_ / self.num_domains
    
class vIRMGAME(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(vIRMGAME, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        
        self.num_domains = num_domains
        self.num_classes = num_classes
        self.period = self.num_domains + 1
        # we use same architecture across environments and store it in a list
        self.model_list = []
        self.optimizer_list = []
        featurizer = networks.Featurizer(input_shape, self.hparams).to('cuda:0')
        for e in range(self.num_domains + 1):
            if (e < self.num_domains):
                classifier = networks.Classifier(
                    featurizer.n_outputs,
                    self.num_classes,
                    self.hparams['nonlinear_classifier']).to('cuda:0')

                classifier = nn.DataParallel(classifier)
                self.model_list.append(classifier)
            else:
                featurizer = nn.DataParallel(featurizer)
                self.model_list.append(featurizer)
            ...
            optimizer = build_optimizer(
                OptimizerSetting(
                    name=self.hparams["optimizer_name"],
                    # lr=self.hparams["lr"] if e < self.num_domains else self.hparams["lr"]*10,
                    lr=self.hparams["lr"], 
                    weight_decay=self.hparams['weight_decay'],
                    model=self.model_list[e],
                    momentum=self.hparams["momentum"], # sgd, momentum_sgd, sgd_nesterov, rmsprop
                    eps=self.hparams["eps"], # adam, rmsprop (term added to the denominator to improve numerical stability)
                    alpha=self.hparams["alpha"], # rmsprop(smoothing constant of 2nd moment)
                    beta_1=self.hparams["beta_1"], # adam (smoothing constant of 1st moment)
    ...
    def predict(self, x):
        y_ = torch.zeros(x.size()[0], self.num_classes, device=x.device) #batchsize * num_classes
        z = self.model_list[self.num_domains](x)
        for e in range(self.num_domains):
            y_ = y_ + self.model_list[e](z)
...
class BIRM(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(BIRM, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.birm_type = hparams["birm_type"]
        self.birm_sd = hparams["birm_sd"]
        self.l2_regularizer_weight = hparams["l2_regularizer_weight"]
        self.num_domains = num_domains
        
        self.ebd = EBD(self.num_domains, num_classes).cuda()
    ...
    def update(self, minibatches, unlabeled=False):
        ...
        if self.birm_type == "bayes_variance":
            sampleN = 5
            train_penalty = 0
            for i in range(sampleN):
                loss_list = []
                # conpute loss on each env
                for i in range(self.num_domains):
                    pred_i = self.network(minibatches[i][0])
                    loss_i = F.cross_entropy(pred_i, minibatches[i][1])
        ...
        elif self.birm_type == "bayes_fullbatch":
            sampleN = 5
            train_penalty = 0
            for i in range(sampleN):
                self.ebd.re_init_with_noise(self.birm_sd)
                loss = 0
                for e in range(self.num_domains):
                    pred_i = self.network(minibatches[e][0])
                    train_g = torch.full(minibatches[e][1].shape, e).cuda() 
                    train_logits_w = self.ebd(train_g)*pred_i
                    loss += F.cross_entropy(train_logits_w, minibatches[e][1])
     
                grad = autograd.grad(
                    loss, self.ebd.parameters(),
                    create_graph=True)[0]
                train_penalty +=  1/sampleN * torch.mean(grad**2)
        
        elif self.birm_type == "bayes_batch":
            sampleN = 5
            train_penalty = 0
            for i in range(sampleN):
                self.ebd.re_init_with_noise(self.birm_sd)
                ...
                for e in range(self.num_domains):
                    ...
                    train_g = torch.full(minibatches[e][1].shape, e).cuda()
                    train_logits_w = self.ebd(train_g)*pred_i
                    penalty += self.birm_batch_penalty(train_logits_w, minibatches[e][1])
                train_penalty +=  1/sampleN * penalty
        ...
        penalty_weight = (self.hparams["irm_lambda"]
            if self.update_count >= self.hparams["irm_penalty_anneal_iters"] else 0.0)
        ...
        return {'loss': loss.item(), 'penalty': train_penalty.item()}
    
    def birm_batch_penalty(self, logits, y):
        loss_1 = F.cross_entropy(logits[::2], y[::2])
        loss_2 = F.cross_entropy(logits[1::2], y[1::2])
        grad_1 = autograd.grad(loss_1, self.ebd.parameters(), create_graph=True)[0]
        grad_2 = autograd.grad(loss_2, self.ebd.parameters(), create_graph=True)[0]
        result = torch.sum(grad_1 * grad_2)
        return result
...
class PAIR(IRM):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(PAIR, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        
        self.num_domains = num_domains
        self.scale = torch.tensor(1.).cuda().requires_grad_()
        self.pair_opt = pair_opt(self.network.parameters(), self.optimizer, preference=self.get_preference(self.hparams["pair_pref"]), eps=self.hparams["eps"]) #todo
        
    
    def IRM_penalty_pair(self, losses):
        grad0 = autograd.grad(losses.mean(), [self.scale], create_graph=True)[0]
        train_penalty = torch.sum(grad0**2)
        return train_penalty

    def get_preference(self, preference_choice):
        # erm, irm, v-rex
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        if self.update_count < self.hparams['irm_penalty_anneal_iters']:
            loss = 0
            for e in range(self.num_domains):
                y_hat = self.network(minibatches[e][0])
                loss += F.cross_entropy(self.scale * y_hat,minibatches[e][1])
            ...
            for e in range(self.num_domains):
                y_hat = self.network(minibatches[e][0])
                loss = F.cross_entropy(self.scale * y_hat,minibatches[e][1],reduction="none")
                losses_bygroup.append(loss.mean())
                penalty += self.IRM_penalty_pair(loss)
                avg_loss += loss.mean()
            
            avg_loss /= self.num_domains
            penalty /= self.num_domains
            losses = [avg_loss, penalty, torch.stack(losses_bygroup).var()]
            
            self.pair_opt.zero_grad()
            self.pair_opt.set_losses(losses)
            pair_loss, moo_losses, mu_rl, alpha, lr = self.pair_opt.step()

            self.update_count += 1
        
            return {'loss': avg_loss.item(), 'penalty': penalty.item(), 'pair_loss': pair_loss.item(), 'alpha': alpha}
