# coding: utf-8
# copied from https://github.com/alecwangcq/KFAC-Pytorch/blob/master/optimizers/kfac.py
import math
import torch
    ...
    def linear(input, grad_output, layer):
        ...
        with torch.no_grad():
            if layer.bias is not None:
                input = torch.cat(
                    [input, input.new(input.size(0), 1).fill_(1)], 1)
    ...
    def conv2d(input, grad_output, layer):
        ...
        with torch.no_grad():
            ...
            if layer.bias is not None:
                input = torch.cat(
                    [input, input.new(input.size(0), 1).fill_(1)], 1)
            input = input.view(grad_output.size(
                0), -1, input.size(-1))  # b * hw * in_c*kh*kw
...
class KFACOptimizer(optim.Optimizer):
    def __init__(self,
        ...
        # super(KFACOptimizer, self).__init__(model, defaults)
    ...
    def _update_inv(self, m):
        ...
        if self.n_distributed is not None:
            print('distributed')
            self.m_aa[m] = hvd.allreduce(self.m_aa[m], average=True)
            self.m_gg[m] = hvd.allreduce(self.m_gg[m], average=True)
    ...
    def step(self, closure=None):
        ...
        if self.n_distributed is not None:
            for p in group['params']:
                p.grad = hvd.allreduce(p.grad, average=True)
