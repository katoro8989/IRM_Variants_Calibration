...
class ConjugateGradientOptimizer(Optimizer):
    ...
    def calculate_beta(self, current_grad, prev_grad, d_update_v, beta_update_rule):
        ...
        if beta_update_rule == 'FR':
            # beta = float(min(float(torch.dot(current_grad, current_grad)) / float(torch.dot(prev_grad, prev_grad)), 1/2))
            beta = 0
            ...
            # beta = min(torch.dot(current_grad, current_grad - prev_grad) / torch.dot(prev_grad, prev_grad), 1/2)
            beta = 0
            denominator = float(torch.dot(prev_grad, prev_grad))
            if denominator != 0:
            ...
            # beta = min(torch.dot(current_grad, current_grad - prev_grad) / torch.dot(d_update_v, current_grad - prev_grad), 1/2)
            beta = 0
            ...
            # beta = min(torch.dot(current_grad, current_grad) / torch.dot(d_update_v, current_grad - prev_grad), 1/2)
            beta = 0
            denominator = float(torch.dot(d_update_v, current_grad - prev_grad))
            if denominator != 0:
                numerator = float(torch.dot(current_grad, current_grad))
            ...
            # beta_hs = min(torch.dot(current_grad, current_grad - prev_grad) / torch.dot(d_update_v, current_grad - prev_grad), 1/2)
            # beta_dy = min(torch.dot(current_grad, current_grad) / torch.dot(d_update_v, current_grad - prev_grad), 1/2)
            ...
            # beta_fr = min(torch.dot(current_grad, current_grad) / torch.dot(prev_grad, prev_grad), 1/2)
            # beta_prp = min(torch.dot(current_grad, current_grad - prev_grad) / torch.dot(prev_grad, prev_grad), 1/2)
