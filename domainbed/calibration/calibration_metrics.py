...
class CalibrationMetric():
    ...
    def __init__(self,
        ...
        if bin_method == "equal_width" or ce_type.startswith("ew"):
            ...
            raise NotImplementedError(
                f"Bin method {bin_method} not supported.")
    ...
    def compute_error(self, fx, y):
        ...
        if np.min(fx) < 0.:
            raise ValueError(
                "Minimum score of {} is less than 0.".format(np.min(fx)))
        if np.max(y) > 1.:
            raise ValueError("Maximum label of {} is greater than 1.".format(
                np.max(y)))
        if np.min(y) < 0.:
            raise ValueError(
                "Minimum label of {} is less than 0.".format(np.min(y)))
        ...
        if self.num_bins > 0 and self.bin_method:
            binned_fx, binned_y, bin_sizes, bin_indices = self.bin_data(fx, y)
