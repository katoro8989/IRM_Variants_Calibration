...
class PAIR(Optimizer):
    ...
    def step(self, closure=None):
        ...
        if len(self.losses) == 0:
            ...
            pair_loss = torch.stack(losses).dot(scales).requires_grad_(True)
            # pair_loss = torch.mean(torch.stack(losses)).requires_grad_(True)
            if reset_optimizer:
            ...
            return pair_loss, moo_losses, mu_rl, alpha[0], self.descent
